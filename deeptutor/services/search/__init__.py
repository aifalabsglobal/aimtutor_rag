...
# Default cap on how many images we attach per query when ``include_images``
# is on. Kept small so the consolidated answer stays under the chat trace
# truncation limit (``MAX_TOOL_RESULT_CHARS = 4000``).
...
def _serpapi_api_key() -> str:
    """Resolve the SerpAPI key (used by ``include_images`` augmentation).

    Looks for ``SERPAPI_API_KEY`` (canonical) and ``SERP_API_KEY`` (a common
    alias). Falls back to the unified ``SEARCH_API_KEY`` when SerpAPI is the
    active default provider.
    """
    store = get_env_store()
    for env_name in ("SERPAPI_API_KEY", "SERP_API_KEY"):
        value = store.get(env_name, "").strip()
        if value:
            return value
    if store.get("SEARCH_PROVIDER", "").strip().lower() == "serpapi":
        return store.get("SEARCH_API_KEY", "").strip()
    return ""


def _augment_with_serpapi_images(
    ...
    *,
    ...
    """If ``SERPAPI_API_KEY`` is set, fetch the top images for ``query`` and
    append them to ``response.metadata['images']`` plus a markdown gallery on
    ``response.answer`` so the LLM (and downstream UI) can reference them.
    """
    ...
    try:
        provider = get_provider("serpapi", api_key=api_key, proxy=proxy)
        image_response = provider.search(query, mode="images", num=max(limit * 2, limit))
    except Exception as exc:  # pragma: no cover - upstream API failure
        _logger.warning(f"SerpAPI image lookup failed: {exc}")
        return response

    images = image_response.metadata.get("images") or []
    images = images[:limit]
    if not images:
        return response

    existing = list(response.metadata.get("images") or [])
    existing_thumbs = {img.get("thumbnail") for img in existing if isinstance(img, dict)}
    merged = existing + [img for img in images if img.get("thumbnail") not in existing_thumbs]
    response.metadata["images"] = merged

    # Append a markdown gallery so the LLM observes the visuals and the chat
    # trace renderer (Markdown) can display them when the model echoes them
    # in its final answer.
    gallery_lines = ["", "## Images"]
    for img in images:
        title = (img.get("title") or img.get("source") or "image").strip()
        thumb = img.get("thumbnail") or img.get("original") or ""
        if not thumb:
            continue
        link = img.get("link") or img.get("original") or thumb
        gallery_lines.append(f"![{title}]({thumb})")
        gallery_lines.append(f"<sub>{title} — [{img.get('source', '')}]({link})</sub>")
        gallery_lines.append("")
    if len(gallery_lines) > 2:
...
def _assert_provider_supported(provider_name: str) -> None:
    if provider_name in _DEPRECATED_UNSUPPORTED:
        raise ValueError(
            ...
            "Please switch to brave, tavily, jina, searxng, duckduckgo, perplexity, serper, or serpapi."
        )
...
def web_search(
    ...
    **provider_kwargs: Any,
    ...
    When ``include_images`` is true and ``SERPAPI_API_KEY`` is configured,
    the response is augmented with a ``metadata['images']`` gallery and a
    markdown image strip appended to ``answer``. This lets the chat agent
    surface visuals alongside the textual web-search summary.
    """
    ...
    if include_images and provider_name != "serpapi":
        # ``serpapi`` already populated metadata['images'] when invoked above
        # (its native search engine returns inline_images). For every other
        # provider, perform a side-channel SerpAPI image lookup so chat can
        # always surface visuals when the user has the key configured.
        response = _augment_with_serpapi_images(
            response,
            query,
            limit=image_limit,
