...
class AgenticChatPipeline:
    ...
    async def _run_native_tool_loop(
        ...
        for tool_index, ((tool_call_id, tool_name, tool_args), tool_result) in enumerate(
            ...
            extras: dict[str, Any] = {}
            if tool_name == "web_search" and isinstance(metadata, dict):
    ...
    def _tool_trace_metadata(
        ...
        *,
        ...
        return merge_trace_metadata(trace_meta, payload)
    ...
    def _augment_tool_kwargs(
        ...
        if tool_name == "rag" and context.knowledge_bases:
            ...
            # Always opt in to SerpAPI image enrichment for chat — the
            # service layer no-ops when ``SERPAPI_API_KEY`` is unset, so
            # this is safe even on minimal deployments.
