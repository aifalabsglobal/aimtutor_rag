...
type TraceMetadata = {
  ...
  images?: Array<SerpImage>;
};

export type SerpImage = {
  thumbnail?: string;
  original?: string;
  title?: string;
  link?: string;
...
function ImageGallery({ images }: { images: SerpImage[] }) {
  // Dedup by thumbnail URL to avoid duplicate cards when the same image
  // appears across multiple tool_result events (e.g. retried searches).
  const seen = new Set<string>();
  const unique: SerpImage[] = [];
  for (const img of images) {
    const key = img.thumbnail || img.original || "";
  ...
  return (
    <div className="mt-1.5 grid grid-cols-3 gap-1.5 sm:grid-cols-4 md:grid-cols-5">
      {unique.map((img, idx) => {
        const thumb = img.thumbnail || img.original || "";
        const link = img.link || img.original || thumb;
        const title = img.title || img.source || "";
        return (
          <a
            key={`gallery-${idx}-${thumb}`}
            href={link}
            target="_blank"
            rel="noopener noreferrer"
            title={title}
            className="group relative block aspect-square overflow-hidden rounded-md border border-[var(--border)] bg-[var(--muted)]/40 transition-transform hover:-translate-y-0.5 hover:border-[var(--primary)]/40"
          >
            ...
            />
            {img.source ? (
              <span className="pointer-events-none absolute bottom-0 left-0 right-0 truncate bg-gradient-to-t from-black/70 to-transparent px-1.5 py-0.5 text-[9px] not-italic text-white/90">
                {img.source}
              </span>
            ) : null}
          </a>
...
function TraceRowBody({
  ...
  const inlineImages: SerpImage[] = callEvents.flatMap((event) => {
    if (event.type !== "tool_result") return [];
    const meta = getTraceMeta(event);
    return Array.isArray(meta.images) ? meta.images : [];
  });
    ...
    <div className="text-[11px] italic leading-[1.6] text-[var(--muted-foreground)]">
      ...
      {inlineImages.length > 0 && (
        <div className="mt-1.5">
          <div className="not-italic text-[10px] font-semibold uppercase tracking-[0.04em] text-[var(--muted-foreground)]/70">
            {t("Images")}
          </div>
...
function hasExpandableContent(
  ...
  const inlineImages = callEvents.flatMap((event) => {
    if (event.type !== "tool_result") return [];
    const meta = getTraceMeta(event);
    return Array.isArray(meta.images) ? meta.images : [];
  });
